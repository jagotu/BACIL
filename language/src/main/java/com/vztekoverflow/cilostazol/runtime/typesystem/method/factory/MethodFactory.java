...
public final class MethodFactory {
  public static IMethod create(CLIMethodDefTableRow mDef, CLIType definingType) {
    final IType[] definingTypeParameters =
        (definingType instanceof NonGenericType) ? new IType[0] : definingType.getTypeParameters();
    ...
    final ITypeParameter[] typeParameters =
        FactoryUtils.getTypeParameters(
            context,
    ...
    if (!flags.hasFlag(MethodFlags.Flag.ABSTRACT)) {
      ...
      if (methodHeaderFlags.hasFlag(MethodHeaderFlags.Flag.CORILMETHOD_FATFORMAT)
        ...
        handlers =
            createHandlers(
                buf, typeParameters, definingTypeParameters, definingType.getCLIComponent());
      } else {
    ...
    IReturnType retType =
        new ReturnType(
            mSignature.getRetType().isByRef(),
            TypeFactory.create(
                mSignature.getRetType().getTypeSig(),
                typeParameters,
                definingTypeParameters,
                definingType.getCLIComponent()));
  ...
  // region Helpers
  private static ILocal[] createLocals(
      LocalVarsSig signatures, IType[] mvars, IType[] vars, CLIComponent component) {
  ...
  private static IExceptionHandler[] createHandlers(
      ByteSequenceBuffer buf, IType[] mvars, IType[] vars, CLIComponent component) {
    final IExceptionHandler[] handlers;
    ...
    for (int i = 0; i < clauses; i++) {
      ...
      final IType klass =
          (flags.hasFlag(ExceptionClauseFlags.Flag.COR_ILEXCEPTION_CLAUSE_EXCEPTION))
              ? TypeFactory.create(
                  CLITablePtr.fromToken(classTokenOrFilterOffset), mvars, vars, component)
              : null;
      handlers[i] =
          new ExceptionHandler(tryoffset, trylength, handleroffset, handlerlength, klass, flags);
    }
    return handlers;
  }

  private static IParameter[] createParameters(
      ParamSig[] params,
    ...
    for (int i = 0; i < params.length; i++) {
      parameters[i] =
          new Parameter(
              params[i].isByRef(),
              TypeFactory.create(params[i].getTypeSig(), mvars, vars, component),
              rows[i].getNameHeapPtr().read(component.getDefiningFile().getStringHeap()),
              rows[i].getSequence(),
              new ParamFlags(rows[i].getFlags()));
    }
