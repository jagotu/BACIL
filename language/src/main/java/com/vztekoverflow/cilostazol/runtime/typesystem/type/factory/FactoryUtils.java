...
public final class FactoryUtils {
  static ITypeParameter[] getTypeParameters(CLITypeDefTableRow ptr, CLIComponent component) {
    ...
    for (CLIGenericParamTableRow row : component.getTableHeads().getGenericParamTableHead()) {
      ...
      if (CLITableConstants.CLI_TABLE_TYPE_DEF == genParamOwner.getTableId()
          && ptr.getRowNo() == genParamOwner.getRowNo()) {
    ...
    for (CLIGenericParamTableRow row : typeParameters) {
      result[row.getNumber()] =
          TypeParameterFactory.create(row, null, result, component, component.getDefiningFile());
    }
  ...
  static boolean interfaceExtendsClass(
    ...
    var potentialExtendingClassRow =
        component.getTableHeads().getTypeDefTableHead().skip(interfaceRow.getKlassTablePtr());
  ...
  static IType getDirectBaseClass(
      CLITypeDefTableRow typeDefRow, IType[] mvars, IType[] vars, CLIComponent component) {
    ...
    switch (tablePtr.getTableId()) {
      ...
      case CLITableConstants.CLI_TABLE_TYPE_REF -> baseClass =
          TypeFactory.create(
              component.getDefiningFile().getTableHeads().getTypeRefTableHead().skip(tablePtr),
              component);
      case CLITableConstants.CLI_TABLE_TYPE_SPEC -> baseClass =
          TypeFactory.create(
              component.getDefiningFile().getTableHeads().getTypeSpecTableHead().skip(tablePtr),
              mvars,
              vars,
              component);
      default -> throw new NotImplementedException();
  ...
  static IType getTypeFromDifferentModule(
      CLIComponent component, String name, String namespace, CLITablePtr resolutionScopeTablePtr) {
    // TODO: I feel like getting tables should be done via something else that component -> some
    // assembly appDomain/Assembly singleton maybe?
    var moduleRefName =
        component.getNameFrom(
            component.getTableHeads().getModuleRefTableHead().skip(resolutionScopeTablePtr));

    // We can omit looking for file since we know that it is in the same assembly as this
    var definingComponent =
        Arrays.stream(component.getDefiningAssembly().getComponents())
            .filter(c -> c.getDefiningFile().getName().equals(moduleRefName))
            .findFirst()
            .orElseThrow(
                () ->
                    new TypeSystemException(
                        CILOSTAZOLBundle.message(
                            "cilostazol.exception.moduleRefNotFound", moduleRefName)));
    return definingComponent.getLocalType(name, namespace);
  }

  static IType getTypeFromDifferentAssembly(
      CLIComponent component, String name, String namespace, CLITablePtr resolutionScopeTablePtr) {
    var referencedAssemblyIdentity =
        AssemblyIdentity.fromAssemblyRefRow(
            component.getDefiningFile().getStringHeap(),
            component.getTableHeads().getAssemblyRefTableHead().skip(resolutionScopeTablePtr));
    var referencedAssembly =
        component.getDefiningAssembly().getAppDomain().getAssembly(referencedAssemblyIdentity);
