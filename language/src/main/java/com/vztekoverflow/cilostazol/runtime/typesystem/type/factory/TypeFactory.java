...
public final class TypeFactory {
  public static IType create(CLITablePtr ptr, IType[] mvars, IType[] vars, CLIComponent component) {
    return switch (ptr.getTableId()) {
      case CLITableConstants.CLI_TABLE_TYPE_DEF -> component.getLocalType(ptr);
      case CLITableConstants.CLI_TABLE_TYPE_REF -> TypeFactory.create(
          component.getDefiningFile().getTableHeads().getTypeRefTableHead().skip(ptr), component);
      case CLITableConstants.CLI_TABLE_TYPE_SPEC -> TypeFactory.create(
          component.getDefiningFile().getTableHeads().getTypeSpecTableHead().skip(ptr),
          mvars,
          vars,
          component);
  ...
  public static IType create(CLITypeDefTableRow typeDefRow, CLIComponent component) {
    ...
    IType directBaseClass =
        FactoryUtils.getDirectBaseClass(typeDefRow, new IType[0], genericParameters, component);
  ...
  public static IType create(
      CLITypeSpecTableRow type,
      IType[] methodTypeParameters,
      IType[] classTypeParameters,
  ...
  public static IType create(CLITypeRefTableRow type, CLIComponent component) {
    ...
    return switch (resolutionScopeTablePtr.getTableId()) {
      ...
      case CLITableConstants.CLI_TABLE_MODULE_REF -> FactoryUtils.getTypeFromDifferentModule(
          component, name, namespace, resolutionScopeTablePtr);
      case CLITableConstants.CLI_TABLE_MODULE -> throw new InvalidCLIException();
      case CLITableConstants.CLI_TABLE_ASSEMBLY_REF -> FactoryUtils.getTypeFromDifferentAssembly(
          component, name, namespace, resolutionScopeTablePtr);
      default -> throw new TypeSystemException(
  ...
  public static IType create(
      TypeSpecSig typeSig, IType[] mvars, IType[] vars, CLIComponent definingComponent) {
  ...
  public static IType create(
      TypeSig typeSig, IType[] mvars, IType[] vars, CLIComponent definingComponent) {
    // TODO: null reference exception might have occured here if TypeSig is not created from CLASS
    // TODO: resolve for other types (SZARRAY, GENERICINST, ...)
