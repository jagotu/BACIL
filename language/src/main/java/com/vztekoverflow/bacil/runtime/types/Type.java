...
/**
 * A generic BACIL type. A combination of methods, static and instance fields, type metadata and
 * information on how to move the type between storage states.
 *
 * <p>Every typed value can exist in three storage states: on the evaluation stack, in a location
 * (local variables, arguments, fields) and as a Java object (used for Truffle arguments). Each type
 * must implement transitions from every storage states to every other state.
 */
public abstract class Type {
  ...
  @CompilerDirectives.CompilationFinal protected boolean inited = false;
  ...
  @CompilerDirectives.CompilationFinal protected LocationsDescriptor instanceFieldsDescriptor;

  @CompilerDirectives.CompilationFinal(dimensions = 1)
  protected TypedField[] staticFields = null;

  @CompilerDirectives.CompilationFinal protected LocationsDescriptor staticFieldsDescriptor;

  @CompilerDirectives.CompilationFinal protected LocationsHolder staticFieldsHolder;
  ...
  public void staticFieldToStackVar(TypedField field, Object[] refs, long[] primitives, int slot) {
    staticFieldsDescriptor.locationToStack(
        staticFieldsHolder, field.getIndex(), refs, primitives, slot);
  }
  ...
  public void instanceFieldToStackVar(
    ...
    instanceFieldsDescriptor.locationToStack(
        obj.getFieldsHolder(), field.getIndex(), refs, primitives, slot);
  }
  ...
  public void instanceFieldFromStackVar(
    ...
    StaticObject obj = (StaticObject) object;
    instanceFieldsDescriptor.stackToLocation(
        obj.getFieldsHolder(), field.getIndex(), ref, primitive);
  }
  ...
  public TypedField getTypedField(CLITablePtr token, CLIComponent callingComponent) {
    if (token.getTableId() == CLITableConstants.CLI_TABLE_MEMBER_REF) {
      CLIMemberRefTableRow row =
          callingComponent.getTableHeads().getMemberRefTableHead().skip(token);
  ...
  public LocationReference getStaticFieldReference(TypedField field) {
    return new LocationReference(
        staticFieldsHolder,
        staticFieldsDescriptor.getPrimitiveOffset(field.getIndex()),
        staticFieldsDescriptor.getRefOffset(field.getIndex()),
        field.getType());
  }
  ...
  public LocationReference getInstanceFieldReference(TypedField field, Object object) {
    ...
    return new LocationReference(
        obj.getFieldsHolder(),
        instanceFieldsDescriptor.getPrimitiveOffset(field.getIndex()),
        instanceFieldsDescriptor.getRefOffset(field.getIndex()),
        field.getType());
  }
  ...
  /**
   * Get the vtable slot identities defined in this type. Each vtable slot represents one virtual
   * method defined in this type and its parents.
   */
  ...
  /**
   * Get the vtable slots defined in this type. Each vtable slot contains the implementation of a
   * virtual method defined in this type and its parents.
   */
