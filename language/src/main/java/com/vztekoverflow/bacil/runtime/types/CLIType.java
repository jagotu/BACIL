...
public class CLIType extends Type {
  ...
  /**
   * Create a new CLIType based on the TypeDef metadata row. This is protected as consumers should
   * call {@link CLIType#fromTypeDef(CLITypeDefTableRow, CLIComponent)}.
   *
  ...
  protected CLIType(CLITypeDefTableRow type, CLIComponent component) {
    ...
    if (type.hasNext()) {
      ...
      methodsEnd =
          component.getTablesHeader().getRowCount(CLITableConstants.CLI_TABLE_METHOD_DEF) + 1;
  ...
  public static CLIType fromTypeDef(CLITypeDefTableRow type, CLIComponent component) {
    ...
    if (type.getExtends().getRowNo() != 0
        && component.getType(type.getExtends()) == component.getBuiltinTypes().getValueTypeType()) {
      return new SystemValueTypeType(type, component);
    }
   ...
   *
   * <ol>
   *   <li>Initializes parent class
   *   <li>Prepares fields descriptors, inheriting parent fields
   *   <li>Initializes static fields
   *   <li>Prepares and fills vtables slots, including parent virtual methods
   *   <li>Calls the class initializer (.cctor)
   * </ol>
   */
  public void init() {
    if (!inited) {
      ...
      for (int i = 0; i < totalFields; i++) {
        ...
        if ((currField.getFlags() & TypedField.FIELD_ATTRIBUTE_STATIC) != 0) {
          field =
              new TypedField(
                  currField.getFlags(),
                  currField.getName().read(definingComponent.getStringHeap()),
                  FieldSig.read(
                      currField.getSignature().read(definingComponent.getBlobHeap()),
                      definingComponent),
                  staticFieldsIndex);
          ...
          field =
              new TypedField(
                  ...
                  FieldSig.read(
                      currField.getSignature().read(definingComponent.getBlobHeap()),
                      definingComponent),
                  instanceFieldsIndex + parentInstanceFields);
      ...
      this.instanceFieldsDescriptor =
          LocationsDescriptor.forFields(
              extendz == null ? null : extendz.instanceFieldsDescriptor, instanceFields);
      ...
      while (currMethod.getRowNo() < methodsEnd) {
        if ((currMethod.getFlags() & CILMethod.METHODATTRIBUTE_VIRTUAL)
            == CILMethod.METHODATTRIBUTE_VIRTUAL) {
          ...
          if (!found) {
            // New vtable slot
            identities.add(
                new VtableSlotIdentity(virtualMethod.getName(), virtualMethod.getSignature()));
      ...
      // 4. Call cctor
      MethodDefSig CCTOR_SIGNATURE =
          new MethodDefSig(
              false,
              false,
              (byte) 0,
              -1,
              definingComponent.getBuiltinTypes().getVoidType(),
              new Type[0]);
  ...
  public BACILMethod getMemberMethod(String name, MethodDefSig signature) {
    ...
    while (curr.getRowNo() < methodsEnd) {
      if (curr.getName().read(definingComponent.getStringHeap()).equals(name)
          && MethodDefSig.read(
                  curr.getSignature().read(definingComponent.getBlobHeap()), definingComponent)
              .compatibleWith(signature)) return definingComponent.getLocalMethod(curr, this);
