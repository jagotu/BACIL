...
public class InternalCallFinder {
   ...
   */
  public static BACILMethod FindInternalCallMethod(
      CLIComponent definingComponent, CLIMethodDefTableRow method, Type type) {
    ...
    if (type instanceof CLIType) {
      ...
      if (namedType.getNamespace().equals("System") && namedType.getName().equals("Math")) {
        if (methodName.equals("Sqrt")) {
          return new MathSqrtMethod(
              definingComponent.getBuiltinTypes(), definingComponent.getLanguage(), type);
        } else if (methodName.equals("Abs")) {
          return new MathAbsMethod(
              definingComponent.getBuiltinTypes(), definingComponent.getLanguage(), type);
        } else if (methodName.equals("Cos")) {
          return new MathCosMethod(
              definingComponent.getBuiltinTypes(), definingComponent.getLanguage(), type);
        }
      } else if (namedType.getNamespace().equals("System.Runtime.CompilerServices")
          && namedType.getName().equals("RuntimeHelpers")) {
        if (methodName.equals("InitializeArray")) {
          return new InitializeArrayMethod(
              definingComponent.getBuiltinTypes(), definingComponent.getLanguage(), type);
        }
      } else if (namedType.getNamespace().equals("System")
          && namedType.getName().equals("ValueType")) {
        if (methodName.equals("GetHashCode")) {
          return new ValueTypeGetHashCodeMethod(
              definingComponent.getBuiltinTypes(), definingComponent.getLanguage(), type);
        }
      }
    }
