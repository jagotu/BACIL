...
public class BACILContext {
  ...
  @CompilerDirectives.CompilationFinal private BuiltinTypes builtinTypes = null;
  @CompilerDirectives.CompilationFinal private boolean builtinInitalizing = false;
  ...
  public CLIComponent loadAssembly(Source source) {
    ...
    if (source.hasBytes()) {
      ...
      throw new BACILParserException(
          "Unexpected character-based source with mime type: " + source.getMimeType());
    } else {
      throw new BACILParserException(
          "Should not reach here: Source is neither char-based nor byte-based!");
    }
    ...
    // If we didn't yet initialize builtin types, use the fact that the first reference in an
    // assembly is
    // always the corelib reference, and resolve them from there.
    if (builtinTypes == null && !builtinInitalizing) {
      builtinInitalizing = true;
      AssemblyIdentity corLibRef =
          AssemblyIdentity.fromAssemblyRefRow(
              c.getStringHeap(), c.getTableHeads().getAssemblyRefTableHead());
