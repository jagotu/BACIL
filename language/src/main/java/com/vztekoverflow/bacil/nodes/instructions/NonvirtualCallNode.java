...
/**
 * A Truffle node representing the call instruction and callvirt <b>only when the target method is
 * not actually virtual</b>. (III.3.19 call – call a method) (III.4.2 callvirt – call a method
 * associated, at runtime, with an object) Stores the resolved string from the instruction.
 */
public class NonvirtualCallNode extends EvaluationStackAwareNode {
  ...
  public NonvirtualCallNode(BACILMethod method, int top) {
    ...
    this.top = top;
    directCallNode =
        com.oracle.truffle.api.nodes.DirectCallNode.create(this.method.getMethodCallTarget());
  }
