...
/**
 * A Truffle node representing a {@link CILMethod} body. Directly interprets simple instructions and
 * nodeizes complex ones.
 *
 * <p>The special TRUFFLE_NODE instruction is used to replace nodeized instructions.
 *
 * <p>The evaluation stack is represented as two arrays, one for primitives ({@code long[]}) and one
 * for references ({@code Object[]}.
 */
public class BytecodeNode extends Node {
  ...
  /**
   * Create a new {@code BytecodeNode} for the specified {@link CILMethod}. The method body is
   * provided as a byte[].
   *
   * <p>Stores arguments and variables in a single {@link LocationsHolder}, variables first and
   * arguments last.
   *
  ...
  /**
   * Get a managed reference (type {@code &}) to the specified location as a {@link
   * LocationReference}
   *
   ...
   */
  private static LocationReference getLocalReference(
      LocationsDescriptor descriptor, LocationsHolder holder, int index) {
    return new LocationReference(
        holder,
        descriptor.getPrimitiveOffset(index),
        descriptor.getRefOffset(index),
  ...
  /**
   * Prepare arguments for calling a method, taking them from the evaluation stack and putting them
   * in an Object[].
   *
   ...
   * @return an array of objects representing the arguments that can be used to call the target
   *     method
   */
  @ExplodeLoop
  public static Object[] prepareArgs(
      long[] primitives, Object[] refs, int top, BACILMethod method, int skip) {
    ...
    for (int i = skip; i < argsCount; i++) {
      args[i] =
          targetTypes[varsCount + i].stackToObject(refs[firstArg + i], primitives[firstArg + i]);
    }
   ...
   *
   * <p>Stack transition: ..., array, index → ..., value
   *
  ...
  public static void storeArrayElem(Type elementType, long[] primitives, Object[] refs, int top) {
    ...
    elementType.stackToLocation(
        array.getFieldsHolder(), index, index, refs[top - 1], primitives[top - 1]);
  }
   ...
   */
  private static boolean binaryCompareResult(
      int opcode, long[] primitives, Object[] refs, int slot1, int slot2) {

    if (EvaluationStackPrimitiveMarker.isEvaluationStackPrimitiveMarker(refs[slot1])
        && EvaluationStackPrimitiveMarker.isEvaluationStackPrimitiveMarker(refs[slot2])) {
      // comparing primitives
      EvaluationStackPrimitiveMarker resultType =
          binaryNumericResultTypes[((EvaluationStackPrimitiveMarker) refs[slot1]).getTag()][
              ((EvaluationStackPrimitiveMarker) refs[slot2]).getTag()];
      if (resultType == null) {
        CompilerDirectives.transferToInterpreterAndInvalidate();
   ...
   * @param slot2 the slot the second value is in
   */
  private static void doCompareBinary(
      int opcode, long[] primitives, Object[] refs, int slot1, int slot2) {
  ...
  /**
   * Do a binary integer operation (and, or, xor) on values on the evaluation stack and put the
   ...
   * @param slot2 the slot the second value is in
   */
  private static void doIntegerBinary(
      int opcode, long[] primitives, Object[] refs, int slot1, int slot2) {
    if (EvaluationStackPrimitiveMarker.isEvaluationStackPrimitiveMarker(refs[slot1])
        && EvaluationStackPrimitiveMarker.isEvaluationStackPrimitiveMarker(refs[slot2])) {
      // calculating with primitives
      EvaluationStackPrimitiveMarker resultType =
          binaryIntegerResultTypes[((EvaluationStackPrimitiveMarker) refs[slot1]).getTag()][
              ((EvaluationStackPrimitiveMarker) refs[slot2]).getTag()];
      if (resultType == null) {
        CompilerDirectives.transferToInterpreterAndInvalidate();
  ...
  private static void doNot(long[] primitives, Object[] refs, int slot) {
    if (refs[slot] == EvaluationStackPrimitiveMarker.EVALUATION_STACK_INT32) {
      ...
      throw new BACILInternalError(
          "not instruction called on invalid object: " + refs[slot].toString());
    }
  }

  /**
   * Do a binary numeric operation (add, sub, mul, div, rem) on values on the evaluation stack and
   * put the result on the evaluation stack.
   *
   * @param opcode the opcode of the instruction
   * @param primitives primitives on the evaluation stack
   * @param refs references on the evaluation stack
   ...
   * @param slot2 the slot the second value is in
   */
  private static void doNumericBinary(
      int opcode, long[] primitives, Object[] refs, int slot1, int slot2) {
    if (EvaluationStackPrimitiveMarker.isEvaluationStackPrimitiveMarker(refs[slot1])
        && EvaluationStackPrimitiveMarker.isEvaluationStackPrimitiveMarker(refs[slot2])) {
  ...
  /**
   * Do a numeric shift operation using values on the evaluation stack and put the result on the
   * evaluation stack.
   *
   * @param opcode the opcode of the instruction
   * @param primitives primitives on the evaluation stack
   ...
   * @param slot2 the slot the second value is in
   */
  private static void doShiftBinary(
      int opcode, long[] primitives, Object[] refs, int slot1, int slot2) {
    ...
    switch (opcode) {
      ...
      case SHR_UN:
        ...
        break;
      case SHR:
        if (refs[slot1] == EvaluationStackPrimitiveMarker.EVALUATION_STACK_INT32)
          primitives[slot1] = TypeHelpers.signExtend32(primitives[slot1]);
        primitives[slot1] = primitives[slot1] >> primitives[slot2];
        break;
    }
  }
   ...
   */
  private static void loadIndirect(
      long[] primitives, Object[] refs, int slot, LocationReference locationReference, Type type) {
    // type safety check should be here, comparing compatibility of the reference type and the type
    // from the instruction
    type.locationToStack(
        locationReference.getHolder(),
        locationReference.getPrimitiveOffset(),
        locationReference.getRefOffset(),
        refs,
        primitives,
        slot);
  }
   ...
   * @param type expected type of the value
   */
  private static void storeIndirect(
      long primitive, Object ref, LocationReference locationReference, Type type) {
    // type safety check should be here, comparing compatibility of the reference type and the type
    // from the instruction
    type.stackToLocation(
        locationReference.getHolder(),
        locationReference.getPrimitiveOffset(),
        locationReference.getRefOffset(),
        ref,
   ...
   */
  private static void storeStack(
      long[] primitives,
      Object[] refs,
      int slot,
      LocationsDescriptor descriptor,
  ...
  public Object execute(VirtualFrame frame) {
    ...
    // We use two arrays to represent the evaluation stack, one for primitives and one for
    // references.
    // For references, the ref is stored directly in refs[] and the slot in primitives[] is
    // undefined.
    ...
    while (true) {
      ...
      // the stack) at any given point in a program shall be identical for all possible control flow
      // paths.
      ...
      System.out.println(
          String.format(
              "Unsupported opcode %02x (%s) in %s (offset %x)",
              curOpcode, BytecodeInstructions.getName(curOpcode), method, pc));
      // Print all executed instructions for debugging
      // System.out.printf("%s:%04x %s\n", method.getName(), pc,
      // BytecodeInstructions.getName(curOpcode));
      ...
      switch (curOpcode) {
        ...
        case INITOBJ:
          // > If typeTok is a value type, the initobj instruction initializes each field of dest to
          // null
        ...
        case LDC_I4_7:
        case LDC_I4_8:
          putInt32(primitives, refs, top, curOpcode - LDC_I4_0);
          break;

        case LDC_I4:
          putInt32(primitives, refs, top, bytecodeBuffer.getImmInt(pc));
          break;
        case LDC_I4_S:
          putInt32(primitives, refs, top, bytecodeBuffer.getImmByte(pc));
          break;
        case LDC_I8:
          putInt64(primitives, refs, top, bytecodeBuffer.getImmLong(pc));
          break;
        case LDC_R4:
          putFloat(primitives, refs, top, Float.intBitsToFloat(bytecodeBuffer.getImmInt(pc)));
          break;
        case LDC_R8:
          putFloat(primitives, refs, top, Double.longBitsToDouble(bytecodeBuffer.getImmLong(pc)));
          break;
        ...
        case STLOC_2:
        case STLOC_3:
          storeStack(primitives, refs, top - 1, descriptor, locations, curOpcode - STLOC_0);
          break;
        case STLOC_S:
          storeStack(
              primitives, refs, top - 1, descriptor, locations, bytecodeBuffer.getImmUByte(pc));
          break;
        ...
        case LDLOC_2:
        case LDLOC_3:
          loadStack(primitives, refs, top, descriptor, locations, curOpcode - LDLOC_0);
          break;
        case LDLOC_S:
          loadStack(primitives, refs, top, descriptor, locations, bytecodeBuffer.getImmUByte(pc));
          break;

        case LDLOCA_S:
          refs[top] = getLocalReference(descriptor, locations, bytecodeBuffer.getImmUByte(pc));
          break;
        ...
        case LDARG_2:
        case LDARG_3:
          loadStack(primitives, refs, top, descriptor, locations, varsCount + curOpcode - LDARG_0);
          break;
        case LDARG_S:
          loadStack(
              primitives,
              ...
              descriptor,
              locations,
              varsCount + bytecodeBuffer.getImmUByte(pc));
          break;
        case LDARGA_S:
          refs[top] =
              getLocalReference(descriptor, locations, varsCount + bytecodeBuffer.getImmUByte(pc));
          break;

        case LDTOKEN:
          refs[top] =
              new CLIComponentTablePtr(bytecodeBuffer.getImmToken(pc), method.getComponent());
          break;
        ...
        case STIND_REF:
          storeIndirect(
              primitives[top - 1],
              refs[top - 1],
              (LocationReference) refs[top - 2],
              builtinTypes.getForTypedOpcode(curOpcode));
          break;
        ...
        case LDELEM_R8:
        case LDELEM_REF:
          loadArrayElem(builtinTypes.getForTypedOpcode(curOpcode), primitives, refs, top);
          break;
        ...
        case STELEM_R8:
        case STELEM_REF:
          storeArrayElem(builtinTypes.getForTypedOpcode(curOpcode), primitives, refs, top);
          break;
        ...
        case LDELEMA:
          top =
              nodeizeOpArr(
                  ...
                  refs,
                  top,
                  method.getComponent().getType(bytecodeBuffer.getImmToken(pc)),
                  pc,
                  curOpcode);
          break;
        ...
        case NEG:
          doNegate(primitives, refs, top - 1);
          break;
        ...
        case DIV:
        case REM:
          doNumericBinary(curOpcode, primitives, refs, top - 2, top - 1);
          break;

        case AND:
        case OR:
        case XOR:
          doIntegerBinary(curOpcode, primitives, refs, top - 2, top - 1);
          break;

        case NOT:
          doNot(primitives, refs, top - 1);
          break;
        ...
        case CGT_UN:
        case CLT_UN:
          doCompareBinary(curOpcode, primitives, refs, top - 2, top - 1);
          break;

        case SHL:
        case SHR:
        case SHR_UN:
          doShiftBinary(curOpcode, primitives, refs, top - 2, top - 1);
          break;
        ...
        case CONV_U4:
        case CONV_U8:
          doConvertToInt(curOpcode, primitives, refs, top - 1);
          break;

        case CONV_R4:
        case CONV_R8:
          doConvertToFloat(curOpcode, primitives, refs, top - 1);
          break;
        ...
        case LDSFLDA:
          top =
              nodeizeOpFld(
                  frame, primitives, refs, top, bytecodeBuffer.getImmToken(pc), pc, curOpcode);
          break;
        case CALL:
        case CALLVIRT:
        case NEWOBJ:
        ...
        case TRUFFLE_NODE:
          top = nodes[bytecodeBuffer.getImmInt(pc)].execute(frame, primitives, refs);
          break;
  ...
  private int nodeizeOpToken(
    ...
    switch (opcode) {
      ...
      default:
        ...
        throw new BACILInternalError(
            String.format(
                "Can't nodeize opcode %02x (%s) yet.",
                opcode, BytecodeInstructions.getName(opcode)));
    }

    // add the node to children, and patch the bytecode with a TRUFFLE_NODE instruction and the node
    // offset
  ...
  private int nodeizeOpFld(
    ...
    if (token.getTableId() == CLITableConstants.CLI_TABLE_MEMBER_REF) {
      CLIMemberRefTableRow row =
          method.getComponent().getTableHeads().getMemberRefTableHead().skip(token);
      ...
      definingType =
          method
              .getComponent()
              .findDefiningType(
                  method.getComponent().getTableHeads().getFieldTableHead().skip(token));
    } else {
