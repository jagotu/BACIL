...
public class CLITableClassesGenerator {
  ...
  private static final String PACKAGE = "com.vztekoverflow.bacil.parser.cli.tables.generated";
      ...
      case 'h':
        {
          ...
          writer.println(
              String.format("\tpublic final CLI%sHeapPtr get%s() {", streamName, fieldName));
          generatePenaltiesCode(penalties, writer);
      ...
      case 'i':
        {
          if (!fieldType.contains("|")) {
            ...
            writer.println(
                "\t\tif (areSmallEnough(MAP"
                    + nameToConstName(fieldName)
                    + "_TABLES)) {codedValue = getShort(offset);} else {codedValue = getInt(offset);}");
            penalties.tablePenalties.add(
                "if (!areSmallEnough(MAP" + nameToConstName(fieldName) + "_TABLES)) offset += 2;");
