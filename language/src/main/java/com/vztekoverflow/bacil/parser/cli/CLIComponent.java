...
public class CLIComponent extends BACILComponent {
  ...
  public CLIComponent(
    ...
    localMethods =
        new BACILMethod
            [tables.getTablesHeader().getRowCount(CLITableConstants.CLI_TABLE_METHOD_DEF)];
    localDefTypes =
        new Type[tables.getTablesHeader().getRowCount(CLITableConstants.CLI_TABLE_TYPE_DEF)];
    localSpecTypes =
        new Type[tables.getTablesHeader().getRowCount(CLITableConstants.CLI_TABLE_TYPE_SPEC)];
   ...
   */
  public static CLIComponent parseComponent(
      ByteSequence bytes, Source source, BACILContext context) {
    ...
    return new CLIComponent(
        cliHeader, cliMetadata, blobHeap, stringHeap, guidHeap, USHeap, tables, peFile, context);
  }
  ...
  /**
   * Find a method represented by the specified token. This method can be called from a compiled
   * context, but ONLY for local methods that were already resolved before; it probably shouldn't be
   * on your hot path anyway.
   *
   * <p>The token can be a pointer to MethodDef or MemberRef.
   *
  ...
  private BACILMethod getForeignMethod(CLITablePtr token) {
    ...
    return type.getMemberMethod(
        memberRef.getName().read(stringHeap),
        MethodDefSig.read(memberRef.getSignature().read(blobHeap), this));
  }
  ...
  public Type findDefiningType(CLIMethodDefTableRow methodDef) {
    ...
    // Walk the TypeDef table and find the row, for which the methodList is before the target
    // methodDef
  ...
  public BACILMethod getLocalMethod(CLIMethodDefTableRow method, Type type) {
    ...
    if (localMethods[method.getRowNo() - 1] == null) {
      ...
      // internal call methods aren't in the component and we must provide the implementation
      // ourselves
      if (CILMethod.isInternalCall(method)) {
        ...
        if (internalMethod == null) {
          throw new BACILInternalError(
              "Attempted to resolve InternalCall method for which no implementation is available: "
                  + type.toString()
                  + "."
                  + method.getName().read(getStringHeap()));
        } else {
  ...
  public Type getForeignType(CLITypeRefTableRow typeRef) {
    ...
    CLIAssemblyRefTableRow assemblyRef =
        getTableHeads().getAssemblyRefTableHead().skip(typeRef.getResolutionScope());
    AssemblyIdentity assemblyRefIdentity =
        AssemblyIdentity.fromAssemblyRefRow(stringHeap, assemblyRef);
   ...
   *
   * <p>The token can be a pointer to a TypeDef, TypeSpec or TypeRef.
   *
  ...
  public Type findLocalType(String namespace, String name) {
    ...
    for (CLITypeDefTableRow row : getTableHeads().getTypeDefTableHead()) {
      if (row.getTypeNamespace().read(stringHeap).equals(namespace)
          && row.getTypeName().read(stringHeap).equals(name)) return getLocalType(row);
    }

    // Check exported types (II.6.8 Type forwarders)
    for (CLIExportedTypeTableRow row : getTableHeads().getExportedTypeTableHead()) {
      if (row.getTypeNamespace().read(stringHeap).equals(namespace)
          && row.getTypeName().read(stringHeap).equals(name)) {
        CLIAssemblyRefTableRow assemblyRef =
            getTableHeads().getAssemblyRefTableHead().skip(row.getImplementation());
        BACILComponent c =
            context.getAssembly(AssemblyIdentity.fromAssemblyRefRow(stringHeap, assemblyRef));
   ...
   *
   * <p>Token can point to a TypeDef or TypeSpec row.
   *
