...
public abstract class CLITableRow<T extends CLITableRow<T>> implements Iterable<T> {
  ...
  /**
   * Create a properly typed new {@link CLITableRow} pointing to the specified row at a specified
   * byte offset.
   *
   * @param tables {@link CLITables} holding the table data
   * @param cursor byte offset of the row
  ...
  /**
   * Get a (properly typed) pointer to a row that's the specified amount of rows after the current
   * one.
   *
  ...
  /**
   * Get a (properly typed) pointer to a row that's the amount of rows specified in the pointer
   * after the current one. Also checks that the {@link CLITablePtr} points to the correct table.
   *
   * <p>This operation mainly makes sense when the current row is the first row, therefore
   * "resolving" the CLITablePtr.
   *
    ...
    if (getTableId() != ptr.getTableId()) {
      ...
      throw new CILParserException(
          ParserBundle.message(
              "cil.parser.exception.ptr.wronglyTyped", ptr.getTableId(), getTableId()));
    }
  ...
  /**
   * Get an unsigned short at the specified offset of current row. Returns an int cause Java doesn't
   * support unsigned shorts.
   */
  ...
  /**
   * Get an unsigned int at the specified offset of current row. Returns a long cause Java doesn't
  ...
  /**
   * Checks whether the specified tables are small enough to use a 2 byte coded index. Coded indices
   * are described II.24.2.6 #~ stream:
   *
   * <p>If e is a coded index that points into table ti out of n possible tables t0,...,tn-1, then
   * it is stored as e &lt;&lt; (log n) | tag {t0,...,tn-1}[ti] using 2 bytes if the maximum number
  ...
  protected final boolean areSmallEnough(byte[] tables) {
    ...
    for (byte table : tables) {
      if (table != CLITableConstants.CLI_TABLE_NONE_ID
          && this.tables.getTablesHeader().getRowCount(table) >= limit) return false;
    }
    return true;
  }
}
